...
def home(request):
    ...
    topics = Topic.objects.all()[0:5]  # Fetch all topics from the database
    room_messages = Message.objects.filter(Q(room__topic__name__icontains=q))  # Fetch recent messages related to the topic
...
def room(request,pk):
    ...
    room_messages = room.message_set.all()# Fetch messages related to the room
    ...
    if request.method == 'POST':
        ...
        room.participants.add(request.user)  # Add the user to the participants
